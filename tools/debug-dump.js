/**
 * Diagnostic tool: captures one raw packet of each type from the game and
 * dumps hex + candidate offsets so you can verify parser alignment against
 * the official F1 25 UDP spec.
 *
 * Usage:
 *   node tools/debug-dump.js
 *
 * Run with the game active and sending UDP telemetry to 127.0.0.1:20777.
 */
const dgram = require('node:dgram');
require('dotenv').config({ quiet: true });

const { parseHeader, HEADER_SIZE } = require('../src/parsers/header');

const UDP_PORT = Number.parseInt(process.env.UDP_PORT || '20777', 10);
...
console.log('');
console.log('========================================');
console.log('  f1-udp-relay: buffer diagnostic tool');
console.log('========================================');
console.log('');
console.log(`Listening on UDP :${UDP_PORT}...`);
console.log(
  `Waiting for one packet of each type: ${[...TARGET_PACKETS]
    .map((id) => PACKET_NAMES[id])
    .join(', ')}\n`,
);
...
socket.on('message', (buf) => {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log(
    `CAPTURED: Packet ${id} (${name}) — total: ${buf.length} bytes, data: ${dataSize} bytes`,
  );
  ...
  if (capturedCount >= TARGET_PACKETS.size) {
    console.log(`\n\n${'*'.repeat(70)}`);
    console.log('All target packets captured. Summary:');
    console.log('*'.repeat(70));
...
function dumpLapData(buf) {
  ...
  for (let car = 0; car < 2; car++) {
    ...
    console.log('  Checking lap-time candidates (uint32, ~60000-300000ms):');
    ...
    console.log('  Checking float candidates for lapDistance (0-7000):');
    for (let b = 0; b <= 40; b += 4) {
      if (offset + b + 4 <= buf.length) {
        const val = buf.readFloatLE(offset + b);
        if (val > 0 && val < 10000 && Number.isFinite(val)) {
...
function dumpParticipants(buf) {
  ...
  console.log(
    `  Data after count: ${dataAfterCount} bytes. Per participant (÷22): ${perParticipant.toFixed(2)} bytes`,
  );
  ...
  for (let p = 0; p < 2; p++) {
    ...
    console.log('  Scanning for ASCII name runs:');
    let nameStart = -1;
    let nameStr = '';
...
function dumpCarTelemetry(buf) {
  ...
  for (let car = 0; car < 2; car++) {
    ...
    console.log('  Checking uint16 for speed (expected 50-400 km/h):');
...
function dumpCarStatus(buf) {
  ...
  console.log('  Checking floats for fuel (5-110 kg):');
  for (let b = 0; b <= 30; b += 1) {
    if (offset + b + 4 <= buf.length) {
      const val = buf.readFloatLE(offset + b);
      if (val > 5 && val < 120 && Number.isFinite(val)) {
...
function dumpCarDamage(buf) {
  ...
  console.log('  Checking floats for tyre wear (0-100):');
